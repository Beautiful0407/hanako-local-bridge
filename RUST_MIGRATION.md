...
The Rust implementation is currently `2.0.0-alpha.2`.

Alpha 2 contains the Windows bridge, manager, signed updater, embedded installer, and Linux cloud router. The complete local validation chain and live cloud deployment passed on July 19, 2026. Stable installed Windows clients remain on `1.4.9` until the prerelease is validated on clean Windows 10 and Windows 11 systems.

Do not copy the Alpha EXEs over an existing stable installation manually. Use the embedded Alpha installer with an isolated `--install-root`, or publish it through a separate prerelease channel. The installer and updater preserve `config.json`, `data`, `logs`, cloud identity, approvals, execution authorizations, jobs, and update result history.
...
Measured from the rebuilt Windows x64 Alpha 2 release:

```text
hanako-bridge.exe       6,247,936 bytes
hanako-manager.exe      2,291,712 bytes
hanako-maintenance.exe  4,349,952 bytes
runtime ZIP             6,116,364 bytes
embedded installer      8,248,832 bytes
```
...
The integration tests use random loopback ports and temporary roots. The update smoke test installs an Alpha 1 payload, injects the current Alpha 2 maintenance binary, and verifies deterministic self-update handoff instead of relying on a Windows file-lock race.
...
target\release\hanako-maintenance.exe pack `
  ...
  --output build\rust-release-alpha2 `
  --public-key update-public-key.xml `
  --version 2.0.0-alpha.2 `
  --channel alpha `
  --package-url HanakoLocalBridge-2.0.0-alpha.2-win-x64.zip `
  --signing-key "$env:USERPROFILE\.hanako-update-signing\private-key.xml" `
  --notes "Hanako Local Bridge Rust 2.0.0-alpha.2"

$env:HANA_INSTALLER_PAYLOAD = (
  Resolve-Path 'build\rust-release-alpha2\HanakoLocalBridge-2.0.0-alpha.2-win-x64.zip'
).Path
...
1. Publish the signed Alpha 2 installer and manifest to a separate prerelease channel.
