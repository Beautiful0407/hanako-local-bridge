...
The Rust implementation is currently `2.0.0-alpha.9`.

Alpha 9 contains the Windows bridge, manager, signed updater, and embedded installer. Alpha 8 moved the release Bridge to the Windows GUI subsystem so Task Scheduler runs it without allocating a visible console window. Alpha 9 adds a one-minute repeating trigger alongside the logon trigger while preserving `MultipleInstancesPolicy=IgnoreNew`; a force-terminated Bridge therefore returns automatically without creating duplicate healthy instances. Debug builds remain console applications, redirected service-command JSON still works, and closing the manager leaves the independent background Bridge running. The compatible Linux cloud router remains deployed at Alpha 2.

Do not copy the Alpha EXEs over an existing installation manually. Use the embedded Alpha 9 installer. It supports first install and overwrite repair of an existing Node or Alpha installation while preserving `config.json`, `data`, `logs`, cloud identity, approvals, execution authorizations, jobs, and update result history.
...
Measured from the rebuilt Windows x64 Alpha 9 release:

```text
hanako-bridge.exe       6,272,000 bytes
hanako-manager.exe      2,299,904 bytes
hanako-maintenance.exe  5,707,264 bytes
runtime ZIP             6,678,257 bytes
embedded installer      8,901,632 bytes
```
...
The integration tests use random loopback ports and temporary roots. The installer smoke test launches a detached legacy Node process through VBS, verifies Alpha 9 takeover, asserts the installed Bridge PE subsystem is Windows GUI, checks the one-minute task trigger, calls the installed manager repair API, checks overwrite and manager single-instance behavior, closes the manager while confirming the Bridge stays healthy, force-terminates the Bridge, waits for a different process ID to recover without a visible window, and then uninstalls. The update smoke test installs an Alpha 8 payload, injects the current Alpha 9 maintenance binary, and verifies deterministic signed update handoff.
...
target\release\hanako-maintenance.exe pack `
  ...
  --output build\rust-release-alpha9 `
  --public-key update-public-key.xml `
  --version 2.0.0-alpha.9 `
  --channel alpha `
  --package-url HanakoLocalBridge-2.0.0-alpha.9-win-x64.zip `
  --signing-key "$env:USERPROFILE\.hanako-update-signing\private-key.xml" `
  --notes "Hanako Local Bridge Rust 2.0.0-alpha.9: periodic self-healing background service"

$env:HANA_INSTALLER_PAYLOAD = (
  Resolve-Path 'build\rust-release-alpha9\HanakoLocalBridge-2.0.0-alpha.9-win-x64.zip'
).Path
...
1. Publish the signed Alpha 9 installer and manifest to the separate prerelease channel.
