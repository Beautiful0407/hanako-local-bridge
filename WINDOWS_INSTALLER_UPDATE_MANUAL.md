...
Alpha 渠道：2.0.0-alpha.12，公网更新支持断线重试和 Range 断点续传
...
build\rust-release-alpha12\HanakoLocalBridge-2.0.0-alpha.12-win-x64.zip
build\rust-release-alpha12\update-manifest.json
```
...
短暂断线恢复后自动清除旧错误
同目录覆盖安装
桌面和开始菜单快捷方式统一指向 `hanako-bridge.exe`
计划任务启动与恢复
重复点击统一产品入口只保留一个管理器窗口
卸载注册和后台卸载
WebView2 退出延迟时重试清理安装目录
Alpha 11 负载升级到 Alpha 12
公网更新包中途断线后保留部分文件并使用 HTTP Range 续传
显式公网 URL、大小和 SHA256 下载探针
...
另一台已安装旧 Node 或旧 Alpha 的电脑不需要先卸载。直接运行 `HanakoLocalBridge-Setup-2.0.0-alpha.12.exe` 覆盖安装即可，安装器会停止旧任务和进程、等待端口释放、安装统一入口且支持周期自愈的 Rust 服务，并保留现有配置与数据。Alpha 11 可以通过管理器在线更新到 Alpha 12；网络中断时更新器会保留部分 ZIP 并续传，更新后快捷方式和卸载显示信息会自动修复。
