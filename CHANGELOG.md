...
- **占位云端地址诊断**：安装后仍使用 `your-server.example.com` 或旧版
  `YOUR_SERVER_IP` 时，桥显示“未配置”警告并停止无意义的 DNS 重连，管理器引导
  用户填写真实的 `wss://` 云端地址；占位地址不会再显示为红色主机解析异常。
- **本机身份探测跨域**：保留 `/api/client-identity` 的云端主机白名单 CORS，允许
  云端 `/desktop/` 读取回环身份端点，同时拒绝不受信来源；占位主机、真实部署地址
  和相似恶意域名均有回归测试覆盖。
...
- **管理器"连接到云"引导**:全新安装的电脑默认使用占位符服务器地址
  (`your-server.example.com`),此前需要手动编辑 `config.json` 才能连上
  自己的服务器。现在管理器检测到占位符地址时,在概览页显示三步引导
  (填写服务器地址 → 自动连接 → 绑定设备),支持一键填入推荐地址
  (`qhdhana.xyz`),自动归一化为 `wss://<host>/local-bridge/connect` 并
  推导匹配的 alpha 更新清单;保存后自动重启服务,连接成功后直接进入
  设备认领流程。配置真实地址后引导自动隐藏。
...
- **一键设备认领引导**:概览页新增云端连接大卡片,状态文案人性化
  (已连接/等待绑定/未连接/已停用),隐藏 claimToken/凭证等安全术语;
  `pending_claim` 时显示三步引导(打开认领页 → 登录 Hana → 自动完成绑定)
  与 spinner 进度提示,自动轮询绑定结果最多 2 分钟,成功/超时均有提示。
- **管理器界面现代化**:CSS 变量明暗双主题、渐变品牌按钮、圆角卡片与
  徽章、图标导航、设备 ID 胶囊、状态点光晕、页面淡入动画、粘性顶栏。
- **`local_fs.append_base64` 工具**:支持二进制文件分块追加传输。云端
  Agent 单次生成参数有长度上限(实测约 7.5KB 稳定),大文件可拆成多块
  依次追加到同一路径,传输量不再随块数重复膨胀;`expectedSha256` 前置
  校验防并发修改,`finalSha256` 最后一块整体校验。
- **应用图标更新**:采用新版 H 形悬索桥 SVG 设计(白色 H 桥塔 + 金色
  悬索 + 水面倒影),仓库保留 `assets/hanako-local-bridge.svg` 源文件。

### 修复

- **检测并修复失效**:`install_task` 删除计划任务后仅依赖 `schtasks /End`
  停止 bridge,但任务宿主(svchost)启动的进程不会被 `/End` 终止,端口
  8787/8788 一直被占用导致 8 秒等待超时,修复失败且任务已删除。现在
  删除任务后主动通过 tasklist/taskkill 终止所有 `hanako-bridge.exe`
  进程(排除修复调用自身),端口确实释放后再重建任务。
- **配置示例精简**:`config.example.json` 移除 `tunnel`/`service`/重连
  参数等用户无需关心的内部字段,缺失字段由默认值补齐;README 同步。

## 2.0.3 - 2026-08-06

### 新功能

- **应用图标**:新增桥主题应用图标(深蓝渐变 + 桥拱图形,云端-本地连接
  主题,16-256 七尺寸 ICO + 512 PNG)。图标随更新包分发,快捷方式与
  卸载项(DisplayIcon)使用自定义图标;更新器自愈(`repair_shell_integration`)
  重建快捷方式时目标改为托盘管理器 `hanako-manager.exe`,图标优先使用
  `assets/hanako-local-bridge.ico`(缺失时优雅回退)。
