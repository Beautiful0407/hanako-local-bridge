...
use base64::Engine;
use nuphus_desktop_api::input::{self, InputEngine};
use nuphus_desktop_api::platform::WindowManager;
use nuphus_desktop_api::{GfxBackend, Scope, Target};
...
pub async fn execute(name: &str, args: &Value) -> Result<String, String> {
    match name {
        ...
        "desktop_perceive" => Err("desktop_perceive (local OCR) requires a vision-enabled build: cargo build --features nuphus-mcp-core/vision (downloads ONNX Runtime models at runtime)".to_string()),
...
fn encode_png(frame: &nuphus_desktop_api::Frame) -> Result<Vec<u8>, String> {
...
fn output_png(
    path: Option<&str>,
    frame: &nuphus_desktop_api::Frame,
    png: &[u8],
...
async fn screen_size() -> Result<String, String> {
    ...
    let frame = nuphus_desktop_api::capture::capture(&target, Scope::Fullscreen)
        .await
...
async fn screenshot(args: &Value) -> Result<String, String> {
    let frame = match args.get("region") {
        Some(r) if r.is_object() => {
            ...
            let target = dummy_target();
            nuphus_desktop_api::capture::capture(&target, Scope::Element { x, y, w, h })
                .await
                .map_err(|e| format!("region capture failed: {}", e))?
        }
        _ => {
            let target = dummy_target();
            nuphus_desktop_api::capture::capture(&target, Scope::Fullscreen)
                .await
                .map_err(|e| format!("screen capture failed: {}", e))?
        }
    };
...
async fn window_screenshot(args: &Value) -> Result<String, String> {
    ...
    let frame = nuphus_desktop_api::capture::capture(&target, Scope::Window)
        .await
...
async fn perceive(args: &Value) -> Result<String, String> {
    ...
    let output = tokio::task::spawn_blocking(move || {
        nuphus_desktop_api::vision::perceive::perceive_image(&image_path)
    })
    .await
...
async fn mouse_drag(args: &Value) -> Result<String, String> {
    ...
    input::mouse::drag(
        nuphus_desktop_api::Point {
            x: start_x,
            y: start_y,
        },
        nuphus_desktop_api::Point { x: end_x, y: end_y },
    )
    .await
...
async fn input(args: &Value) -> Result<String, String> {
    ...
    let hwnd = match args.get("hwnd").and_then(Value::as_i64) {
        Some(v) => v as isize,
        None => nuphus_desktop_api::platform::foreground_hwnd()
    ...
    match mode {
        "type" => {
            ...
            if text.len() > 500 {
                ...
                let prev = nuphus_desktop_api::clipboard::read_text().ok();
                let paste_result: Result<(), String> = async {
                    nuphus_desktop_api::clipboard::write_text(text)
                ...
                let restore_warning = match &prev {
                    Some(saved) => nuphus_desktop_api::clipboard::write_text(saved)
                        .err()
...
async fn clipboard_clean() -> Result<String, String> {
    nuphus_desktop_api::clipboard::write_text("")
