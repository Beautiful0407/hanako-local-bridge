...
$alpha1Package = Join-Path $buildRoot "rust-release\HanakoLocalBridge-2.0.0-alpha.1-win-x64.zip"
$alpha2Manifest = Join-Path $buildRoot "rust-release-alpha2\update-manifest.json"
...
try {
  ...
  Assert-Path $alpha1Package "Rust Alpha 1 payload is missing."
  Assert-Path $alpha2Manifest "Rust Alpha 2 update manifest is missing."
  ...
  if ($exitCode -ne 0) {
    throw "Installing the Alpha 1 payload failed with exit code $exitCode."
  }
  $alpha1Payload = Get-Content -LiteralPath (Join-Path $installRoot "payload-manifest.json") -Raw | ConvertFrom-Json
  if ($alpha1Payload.version -ne "2.0.0-alpha.1") {
    throw "The update fixture was not installed at Alpha 1."
  }
  ...
  if ((Get-FileHash -LiteralPath $maintenance -Algorithm SHA256).Hash -ne $currentMaintenanceHash) {
    throw "The current maintenance binary was not injected into the Alpha 1 fixture."
  }
  if ($oldMaintenanceHash -eq $currentMaintenanceHash) {
    throw "The Alpha 1 fixture already contains the current maintenance binary."
  }
  ...
  $output = & $maintenance apply `
    ...
    --manifest $alpha2Manifest `
    --expected-version "2.0.0-alpha.2" `
    --test-mode
  if ($LASTEXITCODE -ne 0) {
    throw "Alpha 1 maintenance launcher failed with exit code $LASTEXITCODE."
  }
  $handoff = $output | ConvertFrom-Json
  if (-not $handoff.started) {
    throw "Alpha 1 maintenance launcher did not confirm worker handoff."
  }
  ...
  if (-not $state -or $state.status -ne "succeeded") {
    throw "Alpha 1 to Alpha 2 update did not succeed at $statePath. Last read error: $lastStateError. State: $($state | ConvertTo-Json -Compress)"
  }
  if ($state.installedVersion -ne "2.0.0-alpha.2") {
    throw "Update state did not report Alpha 2."
  }

  $payload = Get-Content -LiteralPath (Join-Path $installRoot "payload-manifest.json") -Raw | ConvertFrom-Json
  if ($payload.version -ne "2.0.0-alpha.2") {
    throw "Installed payload did not advance to Alpha 2."
  }
  ...
  $passed = $true
  Write-Output "Rust Alpha 1 to Alpha 2 update smoke test passed"
} finally {
