...
$alpha13Package = Join-Path $buildRoot "rust-release-alpha13\HanakoLocalBridge-2.0.0-alpha.13-win-x64.zip"
$alpha14Manifest = Join-Path $buildRoot "rust-release-alpha14\update-manifest.json"
...
try {
  ...
  Assert-Path $alpha13Package "Rust Alpha 13 payload is missing."
  Assert-Path $alpha14Manifest "Rust Alpha 14 update manifest is missing."
  ...
  if ($exitCode -ne 0) {
    throw "Installing the Alpha 13 payload failed with exit code $exitCode."
  }
  $alpha13Payload = Get-Content -LiteralPath (Join-Path $installRoot "payload-manifest.json") -Raw | ConvertFrom-Json
  if ($alpha13Payload.version -ne "2.0.0-alpha.13") {
    throw "The update fixture was not installed at Alpha 13."
  }
  ...
  if ((Get-FileHash -LiteralPath $maintenance -Algorithm SHA256).Hash -ne $currentMaintenanceHash) {
    throw "The current maintenance binary was not injected into the Alpha 13 fixture."
  }
  if ($oldMaintenanceHash -eq $currentMaintenanceHash) {
    throw "The Alpha 13 fixture already contains the current maintenance binary."
  }
  ...
  Set-ItemProperty -Path $uninstallKey -Name "DisplayVersion" -Value "2.0.0-alpha.13"

  $output = & $maintenance apply `
    --install-root $installRoot `
    --manifest $alpha14Manifest `
    --expected-version "2.0.0-alpha.14" `
    --test-mode
  if ($LASTEXITCODE -ne 0) {
    throw "Alpha 13 maintenance launcher failed with exit code $LASTEXITCODE."
  }
  $handoff = $output | ConvertFrom-Json
  if (-not $handoff.started) {
    throw "Alpha 13 maintenance launcher did not confirm worker handoff."
  }

  $statePath = [string]$handoff.statePath
  $state = Wait-UpdateState $statePath "Alpha 13 to Alpha 14 update"
  if ($state.status -ne "succeeded") {
    throw "Alpha 13 to Alpha 14 update failed: $($state | ConvertTo-Json -Compress)"
  }
  if ($state.installedVersion -ne "2.0.0-alpha.14") {
    throw "Update state did not report Alpha 14."
  }

  $payload = Get-Content -LiteralPath (Join-Path $installRoot "payload-manifest.json") -Raw | ConvertFrom-Json
  if ($payload.version -ne "2.0.0-alpha.14") {
    throw "Installed payload did not advance to Alpha 14."
  }
  ...
  if ([string]$uninstallProperties.DisplayVersion -ne "2.0.0-alpha.14") {
  ...
  if ($exitCode -ne 0) {
    throw "Installing the rollback Alpha 13 fixture failed with exit code $exitCode."
  }
  ...
  if ($rollbackPayload.version -ne "2.0.0-alpha.13") {
    throw "Shell integration failure did not roll the payload back to Alpha 13."
  }
  if ((Get-FileHash -LiteralPath $rollbackBridge -Algorithm SHA256).Hash -ne $rollbackBridgeBefore) {
    throw "Shell integration failure did not restore the Alpha 13 bridge binary."
  }

  $passed = $true
  Write-Output "Rust Alpha 13 to Alpha 14 update smoke test passed"
} finally {
